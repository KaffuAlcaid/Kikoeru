  ...
  <q-page class="admin-page" :class="{ 'admin-page--with-fixed-actions': hasUnsavedChanges || loading }">
      ...
      <q-form @submit="onSubmitRootFolder">
        ...
        <div class="q-pa-sm">
          ...
          <div class="row justify-end">
            <q-btn type="submit" color="primary" icon="create_new_folder" label="添加" />
          </div>
        </div>
      </q-form>
    </q-card>
    ...
    <q-form @submit="onSubmit">
      <q-card class="q-ma-md" v-show="config.rootFolders.length">
        ...
        <q-list>
          <q-item v-for="(rootFolder, index) in config.rootFolders" :key="rootFolder.name">
            ...
            <q-item-section avatar>
              <q-btn flat round color="red" icon="delete" :aria-label="`删除文件夹 ${rootFolder.name}`" @click="removeFromRootFolders(index)" />
            </q-item-section>
          </q-item>
        </q-list>
      </q-card>
